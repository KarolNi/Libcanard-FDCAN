		...
		for (uint8_t i=0U; i<MAX_FILTER_LIST_SIZE; i++) {
			...
			if (!((cfg->id & CANARD_CAN_FRAME_EFF) || !(cfg->mask & CANARD_CAN_FRAME_EFF))) {
				filter_ptr[num_stdid] = 0x2U << 30 |	//Classic CAN Filter
										...
										(cfg->id   & CANARD_CAN_STD_ID_MASK) << 16 |
										(cfg->mask & 0x7F);
                  ...
                  const FDCanID		   iface)
{
  ...
  if ((timings.bit_rate_prescaler < 1U) || (timings.bit_rate_prescaler > 1024U) ||
      (timings.max_resynchronization_jump_width < 1U) || (timings.max_resynchronization_jump_width > 4U) ||
